...
begin
  master = redis.sentinel('master', options[:master])
  sentinels = redis.sentinel('sentinels', options[:master])
  slaves = redis.sentinel('slaves', options[:master])

  #puts master.inspect
  #puts sentinels.inspect
  #puts slaves.inspect

  quorum        = master['quorum'].to_i
  num_slaves    = master['num-slaves'].to_i
  num_sentinels = master['num-other-sentinels'].to_i + 1
  ...
  num_healthy_slaves = slaves.count do |slave|
    slave_status = slave['flags'].split(',')
    !slave_status.include?('o_down') and
      !slave_status.include?('s_down') and
      !slave_status.include?('disconnected') and
      slave['slave-priority'].to_i > 0 and
      slave['last-ok-ping-reply'].to_i < 5000
  end

  num_healthy_sentinels = sentinels.count do |sentinel|
    sentinel_status = sentinel['flags'].split(',')
